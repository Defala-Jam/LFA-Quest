...
const Statistics: React.FC = () => {
  ...
  const avgTime =
    analytics.tags.reduce((acc, t) => acc + t.avg_time, 0) /
    (analytics.tags.length || 1);
  ...
  const handleReviewTopic = () => {
    ...
    const tagsToReview = sortedTags.slice(
      0,
      Math.max(2, Math.ceil(sortedTags.length * 0.3))
    );
    ...
    const reviewQuestions = allLessons.filter(
      (lesson) =>
        lesson.tags && lesson.tags.some((tag) => tagNames.includes(tag))
    );
        ...
        <div className="statistics-container">
          ...
          <div className="tags-section">
            ...
            <p className="section-description">
              Escolha uma tag para ver informações detalhadas sobre seu
              desempenho
            </p>
                ...
                .map((tag, i) => (
                  <div key={i} className="tag-detail-card">
                    <div className="detail-stats-grid">
                      ...
                      <div className="detail-stat">
                        <h3>Acertos</h3>
                        <p className="stat-value-large correct">
                          {tag.correct}
                        </p>
                      </div>
                      <div className="detail-stat">
                        <h3>Erros</h3>
                        <p className="stat-value-large incorrect">
                          {tag.incorrect}
                        </p>
                      </div>
                    </div>
          ...
          {analytics.tag_most_errors && (
            <div className="suggestion-section">
              <h2>Recomendação</h2>
              <p>
                Você parece ter mais dificuldade em{" "}
                <strong>{analytics.tag_most_errors}</strong>. Que tal revisar as
